describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const prodItems = await page.$$('product-item');
    const shad = await prodItems[0].getProperty('shadowRoot');
    const btn = await shad.$("button");
    await btn.click();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for(let i = 1; i < prodItems.length; i++){
      const shad = await prodItems[i].getProperty('shadowRoot');
      const btn = await shad.$("button");
      await btn.click();
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 0; i < prodItems.length; i++){
      ...
      const btn = await shad.$("button");
      const text = await btn.getProperty("innerText");
      const isRemove = text['_remoteObject'].value;
      if(isRemove != 'Remove from Cart'){
        rememberItems=false;
      }
    }
    const cartCount = await page.$('#cart-count');
    const text = await cartCount.getProperty('innerText');
    if(text['_remoteObject'].value != '20'){
      rememberItems=false;
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 0; i < prodItems.length; i++){
      ...
      const text = await btn.getProperty("innerText");
      const isRemove = text['_remoteObject'].value;
      if(isRemove != 'Add to Cart'){
        rememberItems=false;
      }
    }
    const cartCount = await page.$('#cart-count');
    const text = await cartCount.getProperty('innerText');
    if(text['_remoteObject'].value != '0'){
      rememberItems=false;
    }
    expect(rememberItems).toBe(true);
